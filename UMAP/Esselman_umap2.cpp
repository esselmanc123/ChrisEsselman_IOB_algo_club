...
void similarity(vector<vector<double> > dissimilarity_mat, vector<vector<double> >& blank_matrix, double log_nearest_neighbor, int num_neighbors)
{
    ...
    //For each row in dissimilarity matrix find the nearest neighbor to the row
    for (int i = 0; i < dissimilarity_mat.size(); i++)
    {
        double blank_nearest_neighbor = INFINITY;
        int j_index = 0;
        for (int j = 0; j < dissimilarity_mat[i].size(); j++)
        {
            if (dissimilarity_mat[i][j] < blank_nearest_neighbor && i != j)
            {
                blank_nearest_neighbor = dissimilarity_mat[i][j];
                j_index = j;
            }
        }
    ...
    for (int i = 0; i < dissimilarity_mat.size(); i++)
    {
        ...
        for (int j = 0; j < dissimilarity_mat[i].size(); j++)
        {
            ...
            if (i == j)
            {
                blank_matrix[i].push_back(-1.0);
            }
            else
            {
                //Calculate the similarity
                //Write equation for similarity: e raised to a negative power
                double sim_score = exp(-1.0*(dissimilarity_mat[i][j] - dissimilarity_mat[i][nearest_neighbors[i]])/sigma);
                blank_matrix[i].push_back(sim_score);
            }
        }
    }
}
...
int main(int argc, char **argv) 
{
    ...
    //Calculate log2 of number of neighbors
    double log2_num_neighbors = log2(1.0 * num_neighbors);
    ...
    {
        for (int j = 0; j < the_data[0].size(); j++)
        {
            cout << the_data[i][j] << " ";
        }
        cout << "\n";
    ...
    cout << "The dissimilarity matrix is:\n";
    for (int i = 0; i < dissimilarity_matrix.size(); i++)
    {
        for (int j = 0; j < dissimilarity_matrix[0].size(); j++)
        {
